#ifndef CAFFE_ROI_POOLING_AVE_LAYER_HPP_
#define CAFFE_ROI_POOLING_AVE_LAYER_HPP_
...
/** 
 * @brief Perform ave pooling on regions of interest specified by input, takes
 *        as input N feature maps and a list of R regions of interest.
 *
 *   ROIPoolingAveLayer takes 2 inputs and produces 1 output. bottom[0] is
 ...
 *
 *   For each of the R ROIs, ave-pooling is performed over pooled_h x pooled_w
 ...
 *
 * @param param provides ROIPoolingAveParameter roi_pooling_param,
 *        with ROIPoolingAveLayer options:
...
class ROIPoolingAveLayer : public Layer<Dtype> {
 public:
  explicit ROIPoolingAveLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  ...
  virtual inline const char* type() const { return "ROIPoolingAve"; }
...
#endif  // CAFFE_ROI_POOLING_AVE_LAYER_HPP_
